<template>
  <main v-if="!loading">
    ...
    <div class="category" v-if="auth">
      <h3>Category</h3>
      <input type="text" name="category" @input="handleCat" :value="post.category">
      <select name="category" @change="handleChange">
        <option value=""></option>
    ...
    <input v-else name="title" :value="post.title" placeholder="Title" type="text" @change="handleChange" />

    <span>{{ editedString(post.date, newPost ? null : post.edited) }}</span>

    <p v-if="!auth">{{ post.text }}</p>
    <textarea v-else :value="post.text"
